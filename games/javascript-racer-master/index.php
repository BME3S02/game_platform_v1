...
if(!isset($_GET['backgroundImg']) || !isset($_GET['sprites'])){
      ...
      <?php
      if(isset($_GET['backgroundImg']) || isset($_GET['sprites'])){
        if( ($_GET['backgroundImg'] == 'background2') && ($_GET['sprites'] == 'sprites2') ){
          echo 'displayMessage();';
          echo 'setTimeout(startGame, 2000);';
        }
        if( ($_GET['backgroundImg'] == 'background3') && ($_GET['sprites'] == 'sprites3') ){
          echo 'displayMessage();';
          echo 'setTimeout(startGame, 2000);';
        }
        if( ($_GET['backgroundImg'] == 'background4') && ($_GET['sprites'] == 'sprites4') ){
          echo 'displayMessage();';
          echo 'setTimeout(startGame, 2000);';
        }
        if( ($_GET['backgroundImg'] == 'background5') && ($_GET['sprites'] == 'sprites5') ){
          echo 'displayWinMessage();';
      ...
      function writeWin()
      {
        ...
        var img = document.getElementById("winMessage");
        ctx.drawImage(img, 1, 1, 300, 148);
      ...
      function startGame(){
        ...
        var roadWidth      = <?php if(isset($_GET['roadWidth'])){ echo $_GET['roadWidth']; } else { echo $roadWidth; }?>;                   // actually half the roads width, easier math if the road spans from -roadWidth to +roadWidth
        var segmentLength  = 300;                     // length of a single segment
        var rumbleLength   = 3;                       // number of segments per red/white rumble strip
        var trackLength    = 100;                    // z length of entire track (computed)
        ...
        var maxSpeed       = <?php if(isset($_GET['maxSpeed'])){ echo $_GET['maxSpeed']; } else { echo $maxSpeed; }?>;    // top speed (ensure we can't move more than 1 segment in a single frame to make collision detection easier)
        var accel          =  maxSpeed/5;             // acceleration rate - tuned until it 'felt' right
        var breaking       = -maxSpeed;               // deceleration rate when braking
        var decel          = -maxSpeed/5;             // 'natural' deceleration rate when neither accelerating, nor braking
        var offRoadDecel   = -maxSpeed/2;             // off road deceleration is somewhere in between
        var offRoadLimit   =  maxSpeed/4;             // limit when off road deceleration no longer applies (e.g. you can always go at least this speed even when off road)
        var totalCars      = <?php if(isset($_GET['totalCars'])){ echo $_GET['totalCars']; } else { echo $totalCars; }?>;                     // total number of cars on the road
