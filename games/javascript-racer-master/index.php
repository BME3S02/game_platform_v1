...
<!-- ***************************************************************** -->
    ...
    function displayToast(msg, time) {
      ...
      toast.html(msg);
      toast.addClass("show");
      if(time != "inf") setTimeout(function () { toast.removeClass("show");},time||2000);
    }
  ...
  Dom.on('maxTime',        'change', function(ev) { /*console.log("changing");*/Dom.blur(ev); reset({ maxTime:       Util.limit(Util.toInt(ev.target.value), Util.toInt(ev.target.getAttribute('min')), Util.toInt(ev.target.getAttribute('max'))) }); });
